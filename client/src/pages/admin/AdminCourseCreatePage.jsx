...
export default function AdminCourseCreatePage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setSuccess(`Course "${response.data.course.title}" created successfully.`);
      navigate('/admin/courses', {
        replace: true,
        state: { createdCourseTitle: response.data.course.title },
      });
    } catch (err) {
  ...
  return (
    <div className="admin-course-flow">
      ...
      {success ? (
        <p className="admin-course-notice" role="status">{success}</p>
      ) : null}
